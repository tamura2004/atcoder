...
main=do
 ...
 interact ((\input->let output=(let intMatrix_=filter (not . null) (map (asInts.sToA) (lines $ aToS input));strLines=map sToA $ lines $ aToS input;intList=concat intMatrix_;datOverride=False;dat=0;autoMapList=[];in let (firstSep,secondSep)=if length autoMapList > 1 then ([],[space]) else ([space],[newli]) in (let (_,fstInt,fstLine,ints,sndInt,allLines,allInput,intMatrix,sndLine)=(undefined,if datOverride then dat else fromMaybe 100 $ at intList 0,fromMaybe printables $ at strLines 0,intList,fromMaybe 1000 $ at intList 1,strLines,input,intMatrix_,fromMaybe [] $ at strLines 1) in (let (larg0t1,larg0t2)=((\x->((),x)) (((\a1->concat a1).(\a->map (\a1->a1)a)) ((intMatrix) ::[[Integer]]))) in (let (larg1t1,larg1t2)=((\x->((),x)) larg0t2) in (( id .((\a b->intercalate a $ map (sToA.show.confirmInt) b)secondSep)) larg1t2)))))
