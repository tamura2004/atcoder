...
main=do
 ...
 interact ((\input->let output=(let intMatrix_=filter (not . null) (map (asInts.sToA) (lines $ aToS input));strLines=map sToA $ lines $ aToS input;intList=concat intMatrix_;datOverride=False;dat=0;autoMapList=[];in let autoMapList = (listOr [[]] (chunksOf 1 intList)) in concat $ map finishLn $ flip map autoMapList $ \intList -> let (firstSep,secondSep)=if length autoMapList > 1 then ([],[space]) else ([space],[newli]) in (let (_,fstInt,fstLine,ints,sndInt,allLines,allInput,intMatrix,sndLine)=(undefined,if datOverride then dat else fromMaybe 100 $ at intList 0,fromMaybe printables $ at strLines 0,intList,fromMaybe 1000 $ at intList 1,strLines,input,intMatrix_,fromMaybe [] $ at strLines 1) in (( id .((\a b->intercalate a $ map (sToA.show.confirmInt) b)secondSep)) (((\i a->if null a then [] else lazyAtMod a (fromIntegral i - 1)) ((fstInt) ::Integer)) ((((tail) (((((\a f->map (map (\(e,_,_)->e)) $groupBy (onToBy (\(_,fa,_)->fa)) $sortOn (\(_,fa,_)->fa) $zip3 a (map f a) [0..]) ((((((\a f->map f a)) (((((\n a->if n>0 then subsequencesN n a else if n==0 then subsequences a else repeatedSubsequencesN (-n) a) ((((id) (((0)) ::Integer))) ::Integer)) ((((reverse) (((((\a b->((\a1->((id) a1))$(:[])(a()))++((\a1->((id) a1))$ id (b()))) ((\()->( ((((id) (((0)) ::Integer))) ::Integer))))) ((\()->( ((((\x->[1..x]) ((((id) (((9)) ::Integer))) ::Integer))) ::[Integer])))))) ::[Integer]))) ::[Integer]))) ::[[Integer]])) ((\(arg4t1)->( (((((\base a->let aflat = (concat.(:[])) a in fromBase (abs base) (if base < 0 then map (\c->fromIntegral $ fromMaybe (fromIntegral c) $ elemIndex c printables) aflat else aflat)) (((10)) ::Integer)) ((arg4t1) ::[Integer]))) ::Integer)))))) ::[Integer])) ((\(arg4t1)->( ((arg4t1) ::Integer)))))) ::[[Integer]]))) ::[[Integer]])))))
