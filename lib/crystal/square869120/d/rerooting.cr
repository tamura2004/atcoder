...
module Square869120
  module D
    struct Rerooting(T)
      ...
      def initialize(@g, @merge, @apply, @unit)
        @dp = Array.new(n, unit)
        @lo = Array.new(n) do |v|
      ...
      def dfs1(v, pv)
        ...
        g[v].each_with_index do |nv, i|
          next if nv == pv
          dfs1(nv, v)
          num = g[v].size - (pv != -1 ? 1 : 0)
          cnt = lo[v][i + 1] = hi[v][i] = apply.call(dp[nv], num)
      ...
      def dfs2(v, pv, from_par)
        ...
        g[v].each_with_index do |nv, i|
          next if nv == pv
          num = g[nv].size - (v != 0 ? 1 : 0)
          cnt = apply.call(merge.call(lo[v][i], hi[v][i + 1]), num)
          dfs2(nv, v, cnt)
        end
      end
    end
