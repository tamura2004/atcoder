  ...
  users.each do |user|
    ...
    problems.each do |problem|
      if problem["order"] == 0
        ...
        rec.start_time = score["epoch_second"]
        rec.score += 1
      else
        score = user_scores.find do |s|
          s["problem_id"] == problem["id"] &&
        ...
        rec.start_time = score["epoch_second"] if rec.start_time == 0
        rec.end_time = score["epoch_second"] if rec.end_time < score["epoch_second"]
        rec.score += 1
      end
